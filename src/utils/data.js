

export const SKILLS = [
    {
        title: "Frontend",
        icon: "https://raw.githubusercontent.com/NikhilCyberk/portfolio/main/src/assets/main_ico/front-end.png",

        skills: [
            { skill: "HTML5", percentage: "80%" },
            { skill: "CSS3", percentage: "90%" },
            { skill: "JavaScript", percentage: "75%" },
            { skill: "React.js", percentage: "80%" },
        ],

    },
    {
        title: "Backend",
        icon: "https://cdn-icons-png.flaticon.com/512/6213/6213731.png",
        skills: [
            { skill: "Node.js", percentage: "50%" },
            { skill: "Express.js", percentage: "30%" },

        ],

    },
    {
        title: "Tools",
        icon: "https://cdn-icons-png.flaticon.com/512/2249/2249538.png",
        skills: [
            { skill: "Git & Github", percentage: "20%" },
            { skill: "VS Code", percentage: "70%" },
            { skill: "Responsive Design", percentage: "75%" },
        ],

    },
    {
        title: "Soft Skills",
        icon: "https://cdn-icons-png.flaticon.com/512/6195/6195629.png",
        skills: [
            { skill: "Problem Solving", percentage: "60%" },
            { skill: "Collaboration", percentage: "70%" },
            { skill: "Attention to Details", percentage: "45%" },

        ],

    },
];


export const PROJECT = [
    {
        title: "Project-1",
        technologies: "html, css, javascript",
        work: [
            "lorem ipsum dolor sit amet, consectetur adipiscing",
            "lorem ipsum dolor sit amet, consectetur adipiscing",
            "lorem ipsum dolor sit amet, consectetur adipiscing",
            "lorem ipsum dolor sit amet, consectetur adipiscing",
        ],
    },
    {
        title: "Project-2",
        technologies: "html, css, javascript",
        work: [
            "lorem ipsum dolor sit amet, consectetur adipiscing",
            "lorem ipsum dolor sit amet, consectetur adipiscing",
            "lorem ipsum dolor sit amet, consectetur adipiscing",
            "lorem ipsum dolor sit amet, consectetur adipiscing",
        ],
    },
    {
        title: "Project-3",
        technologies: "html, css, javascript",
        work: [
            "lorem ipsum dolor sit amet, consectetur adipiscing",
            "lorem ipsum dolor sit amet, consectetur adipiscing",
            "lorem ipsum dolor sit amet, consectetur adipiscing",
            "lorem ipsum dolor sit amet, consectetur adipiscing",
        ],
    },
];